...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="说说-zookeeper-一般都有哪些使用场景" tabindex="-1"><a class="header-anchor" href="#说说-zookeeper-一般都有哪些使用场景"><span>说说 ZooKeeper 一般都有哪些使用场景？</span></a></h2><ul><li>分布式协调：这个其实就是 zk 很经典的一个用法，简单来说，就好比，你系统 A发送个请求到 mq，然后 B消费了之后处理。那 A系统如何指导 B系统的处理结果？用 zk 就可以实现分布式系统之间的协调工作。A系统发送请求之后可以在 zk 上对某个节点的值注册个监听器，一旦 B系统处理完了就修改 zk 那个节点的值，A立马就可以收到通知，完美解决。</li><li>分布所锁：对某一个数据联系发出两个修改操作，两台机器同时收到请求，但是只能一台机器先执行另外一个机器再执行，那么此时就可以使用 zk 分布式锁，一个机器接收到了请求之后先获取 zk 上的一把分布式锁，就是可以去创建一个 znode，接着执行操作，然后另外一个机器也尝试去创建那个 znode，结果发现自己创建不了，因为被别人创建了，那只能等着，等等一个机器执行完了自己再执行。</li><li>配置信息管理：zk 可以用作很多系统的配置信息的管理，比如 kafka，</li></ul><p>storm 等等很多分布式系统都会选用 zk 来做一些元数据，配置信息的管理，包括 dubbo 注册中心不也支持 zk 么。</p><ul><li>HA高可用性：这个应该是很常见的，比如 hdfs，yarn 等很多大数据系统，都选择基于 zk 来开发 HA高可用机制，就是一个重要进程一般会主备两个，主进程挂了立马通过 zk 感知到切换到备份进程。</li></ul><h2 id="说说你们的分布式-session-方案是啥-怎么做的" tabindex="-1"><a class="header-anchor" href="#说说你们的分布式-session-方案是啥-怎么做的"><span>说说你们的分布式 session 方案是啥？怎么做的？</span></a></h2><ul><li>Tomcat + redis</li></ul><p>其实还挺方便的，就是使用 session 的代码跟以前一样，还是基于 tomcat 原生的 session 支持即可，然后就是用一个叫做 tomcat RedisSessionManager 的东西，让我们部署的 tomcat 都将 session 数据存储到 redis 即可.</p><ul><li>Spring Session + redis</li></ul><p>分布式会话的这个东西重耦合在 tomcat，如果我要将 web容器迁移成 jetty，不能重新把 jetty 都配置一遍.</p><p>所以现在比较好用的还是基于 java 的一站式解决方案，使用 spring session 是一个很好的选择，给 spring session 配置基于 redis 来存储 session 数据，然后配置一个 spring session 的过滤器，这样的话，session 相关操作都会交</p><p>给 spring session 来管了。接着在代码中，就是用原生的 session 操作，就是直接基于 spring session 从 redis 中获取数据了。</p><h2 id="分布式事务了解吗" tabindex="-1"><a class="header-anchor" href="#分布式事务了解吗"><span>分布式事务了解吗？</span></a></h2><ul><li>XA方案/两阶段提交方案</li></ul><p>第一个阶段（先询问）</p><p>第二个阶段（再执行）</p><ul><li>TCC方案</li></ul><p>TCC的全程是：Try、Confirm、Cancel 这个其实是用到了补偿的概念，分为了三个阶段</p><p>Try 阶段：这个阶段说的是对各个服务的资源做检测以及对资源进行锁定或者预留</p><p>Confirm 阶段：这个阶段说的是在各个服务中执行实际的操作</p><p>Cancel 阶段：如果任何一个服务的业务方法执行出错，那么这里就需要进行补偿，就是执行已经成功的业务逻辑的回滚操作</p><ul><li>本地消息表</li><li>可靠消息最终一致性方案</li><li>最大努力通知方案</li></ul><h2 id="那常见的分布式锁有哪些解决方案" tabindex="-1"><a class="header-anchor" href="#那常见的分布式锁有哪些解决方案"><span>那常见的分布式锁有哪些解决方案？</span></a></h2><ul><li>Reids 的分布式锁，很多大公司会基于 Reidis 做扩展开发</li><li>基于 Zookeeper</li><li>基于数据库，比如 Mysql</li></ul><h2 id="zk-和-redis-的区别-各自有什么优缺点" tabindex="-1"><a class="header-anchor" href="#zk-和-redis-的区别-各自有什么优缺点"><span>ZK 和 Redis 的区别，各自有什么优缺点？</span></a></h2><p><strong>先说 Redis：</strong></p><ul><li>Redis 只保证最终一致性，副本间的数据复制是异步进行（Set 是写，Get 是读，Reids 集群一般是读写分离架构，存在主从同步延迟情况），主从切换之后可能有部分数据没有复制过去可能会丢失锁情况，故强一致性要求的业务不推荐使用 Reids，推荐使用 zk。</li><li>Redis 集群各方法的响应时间均为最低。随着并发量和业务数量的提升其响应时间会有明显上升（公有集群影响因素偏大），但是极限 qps 可以达到最大且基本无异常。</li></ul><p><strong>再说 ZK：</strong></p><ul><li>使用 ZooKeeper 集群，锁原理是使用 ZooKeeper 的临时节点，临时节点的生命周期在 Client 与集群的 Session 结束时结束。因此如果某个 Client 节点存在网络问题，与 ZooKeeper 集群断开连接，Session 超时同样会导致锁被错误的释放（导致被其他线程错误地持有），因此 ZooKeeper 也无法保证完全一致。</li><li>ZK具有较好的稳定性；响应时间抖动很小，没有出现异常。但是随着并发量和业务数量的提升其响应时间和 qps 会明显下降。</li></ul><h2 id="mysql-如何做分布式锁" tabindex="-1"><a class="header-anchor" href="#mysql-如何做分布式锁"><span>MySQL 如何做分布式锁？</span></a></h2><p><strong>方法一：</strong></p><p>利用 Mysql 的锁表，创建一张表，设置一个 UNIQUE KEY 这个 KEY 就是要锁的 KEY，所以同一个 KEY 在 mysql 表里只能插入一次了，这样对锁的竞争就交给了数据库，处理同一个 KEY 数据库保证了只有一个节点能插入成功，其他节点都会插入失败。</p><p>DB分布式锁的实现：通过主键 id 的唯一性进行加锁，说白了就是加锁的形式是向一张表中插入一条数据，该条数据的 id 就是一把分布式锁，例如当一次请求插入了一条 id 为1 的数据，其他想要进行插入数据的并发请求必须等第一次请求执行完成后删除这条 id 为1 的数据才能继续插入，实现了分布式锁的功能。</p><p><strong>方法二：</strong></p><p>使用流水号+时间戳做幂等操作，可以看作是一个不会释放的锁。</p><h2 id="你了解业界哪些大公司的分布式锁框架" tabindex="-1"><a class="header-anchor" href="#你了解业界哪些大公司的分布式锁框架"><span>你了解业界哪些大公司的分布式锁框架</span></a></h2><ul><li><h2 id="google-chubby" tabindex="-1"><a class="header-anchor" href="#google-chubby"><span><strong>Google:Chubby</strong></span></a></h2></li></ul><p>Chubby是一套分布式协调系统，内部使用 Paxos 协调 Master 与 Replicas。 Chubby lock service 被应用在 GFS, BigTable 等项目中，其首要设计目标是高可靠性，而不是高性能。</p><p>Chubby被作为粗粒度锁使用，例如被用于选主。持有锁的时间跨度一般为小时或天，而不是秒级。</p><p>Chubby对外提供类似于文件系统的 API，在 Chubby创建文件路径即加锁操作。 Chubby使用 Delay 和 SequenceNumber来优化锁机制。Delay 保证客户端异常释放锁时，Chubby仍认为该客户端一直持有锁。Sequence number 指锁的持有者向 Chubby服务端请求一个序号（包括几个属性），然后之后在需要使用锁的时候将该序号一并发给 Chubby 服务器，服务端检查序号的合法性，包括 number 是否有效等。</p><ul><li><h2 id="京东-sharklock" tabindex="-1"><a class="header-anchor" href="#京东-sharklock"><span>**京东 SharkLock **</span></a></h2></li></ul><p>SharkLock 是基于 Redis 实现的分布式锁。锁的排他性由 SETNX原语实现，使用 timeout 与续租机制实现锁的强制释放。</p><ul><li><h2 id="蚂蚁金服-sofajraft-rheakv-分布式锁" tabindex="-1"><a class="header-anchor" href="#蚂蚁金服-sofajraft-rheakv-分布式锁"><span><strong>蚂蚁金服 SOFAJRaft-RheaKV 分布式锁</strong></span></a></h2></li></ul><p>RheaKV 是基于 SOFAJRaft 和 RocksDB 实现的嵌入式、分布式、高可用、强一致的 KV 存储类库。</p><p>RheaKV对外提供 lock 接口，为了优化数据的读写，按不同的存储类型，提供不同的锁特性。RheaKV提供 wathcdog 调度器来控制锁的自动续租机制，避免锁在任务完成前提前释放，和锁永不释放造成死锁。</p><ul><li><h2 id="netflix-curator" tabindex="-1"><a class="header-anchor" href="#netflix-curator"><span><strong>Netflix: Curator</strong></span></a></h2></li></ul><p>Curator 是 ZooKeeper 的客户端封装，其分布式锁的实现完全由 ZooKeeper 完成。</p><p>在 ZooKeeper 创建 EPHEMERAL_SEQUENTIAL节点视为加锁，节点的 EPHEMERAL特性保证了锁持有者与 ZooKeeper 断开时强制释放锁；节点的 SEQUENTIAL特性避免了加锁较多时的惊群效应。</p><h2 id="请讲一下你对-cap-理论的理解" tabindex="-1"><a class="header-anchor" href="#请讲一下你对-cap-理论的理解"><span>请讲一下你对 CAP 理论的理解</span></a></h2><p>在理论计算机科学中，CAP定理（CAP theorem），又被称作布鲁尔定理</p><ul><li>Brewer’s theorem），它指出对于一个分布式计算系统来说，不可能同时满足以下三点：</li><li>Consistency（一致性）指数据在多个副本之间能够保持一致的特性（严格的一致性）</li><li>Availability（可用性）指系统提供的服务必须一直处于可用的状态，每次请求都能获取到非错的响应（不保证获取的数据为最新数据）</li><li>Partition tolerance（分区容错性）分布式系统在遇到任何网络分区故障的时候，仍然能够对外提供满足一致性和可用性的服务，除非整个网络环境都发生了故障</li></ul><p>Spring Cloud 在 CAP法则上主要满足的是 A和 P法则，Dubbo和 Zookeeper 在 CAP法则主要满足的是 C和 P法则。</p><p>CAP仅适用于原子读写的 NOSQL场景中，并不适合数据库系统。现在的分布式系统具有更多特性比如扩展性、可用性等等，在进行系统设计和开发时，我们不应该仅仅局限在 CAP问题上。现实生活中，大部分人解释这一定律时，常常简单的表述为：“一致性、可用性、分区容忍性三者你只能同时达到其中两个，不可能同时达到”。实际上这是一个非常具有误导性质的说法，而且在 CAP理论诞生12 年之后，CAP之父也在2012 年重写了之前的论文。当发生网络分区的时候，如果我们要继续服务，那么强一致性和可用性只能2 选1。也就是说当网络分区之后 P是前提，决定了 P之后才有 C和 A的选择。也就是说分区容错性（Partition tolerance）我们是必须要实现的。</p><h2 id="请讲一下你对-base-理论的理解" tabindex="-1"><a class="header-anchor" href="#请讲一下你对-base-理论的理解"><span>请讲一下你对 BASE 理论的理解</span></a></h2><p>BASE理论由 eBay架构师 Dan Pritchett 提出，在2008 年上被分表为论文，并且 eBay给出了他们在实践中总结的基于 BASE理论的一套新的分布式事务解决方案。</p><p>BASE 是 Basically Available（基本可用）、Soft-state（软状态）和 Eventually Consistent（最终一致性）三个短语的缩写。BASE理论是对 CAP 中一致性和可用性权衡的结果，其来源于对大规模互联网系统分布式实践的总结，是基于 CAP定理逐步演化而来的，它大大降低了我们对系统的要求。 BASE理论的核心思想是即使无法做到强一致性，但每个应用都可以根据自身业务特点，采用适当的方式来使系统达到最终一致性。也就是牺牲数据的一致性来满足系统的高可用性，系统中一部分数据不可用或者不一致时，仍需要保持系统整体“主要可用”。</p><p>针对数据库领域，BASE思想的主要实现是对业务数据进行拆分，让不同的数据分布在不同的机器上，以提升系统的可用性，当前主要有以下两种做法：</p><ul><li>按功能划分数据库</li><li>分片（如开源的 Mycat、Amoeba等）。</li></ul><h2 id="分布式与集群的区别是什么" tabindex="-1"><a class="header-anchor" href="#分布式与集群的区别是什么"><span>分布式与集群的区别是什么？</span></a></h2><p>分布式：一个业务分拆多个子业务，部署在不同的服务器上集群：同一个业务，部署在多个服务器上。比如之前做电商网站搭的 redis 集群以及 solr 集群都是属于将 redis 服务器提供的缓存服务以及 solr 服务器提供的搜索服务部署在多个服务器上以提高系统性能、并发量解决海量存储问</p><p>题。</p><h2 id="请讲一下-base-理论的三要素" tabindex="-1"><a class="header-anchor" href="#请讲一下-base-理论的三要素"><span>请讲一下 BASE 理论的三要素</span></a></h2><p><strong>基本可用</strong></p><p>基本可用是指分布式系统在出现不可预知故障的时候，允许损失部分可用性。但是，这绝不等价于系统不可用。</p><p>比如：</p><ul><li>响应时间上的损失：正常情况下，一个在线搜索引擎需要在0.5 秒之内返回给用户相应的查询结果，但由于出现故障，查询结果的响应时间增加了1~2秒</li><li>系统功能上的损失：正常情况下，在一个电子商务网站上进行购物的时候，消费者几乎能够顺利完成每一笔订单，但是在一些节日大促购物高峰的时候，由于消费者的购物行为激增，为了保护购物系统的稳定性，部分消费者可能会被引导到一个降级页面</li></ul><p><strong>软状态</strong></p><p>软状态指允许系统中的数据存在中间状态，并认为该中间状态的存在不会影响系统的整体可用性，即允许系统在不同节点的数据副本之间进行数据同步的过程存在延时。</p><p><strong>最终一致性</strong>强调的是系统中所有的数据副本，在经过一段时间的同步后，最终能够达到一个一致的状态。因此，最终一致性的本质是需要系统保证最终数据能够达到一致，而不需要实时保证系统数据的强一致性。</p><h2 id="请说一下对两阶段提交协议的理解" tabindex="-1"><a class="header-anchor" href="#请说一下对两阶段提交协议的理解"><span>请说一下对两阶段提交协议的理解</span></a></h2><p>分布式系统的一个难点是如何保证架构下多个节点在进行事务性操作的时候保持一致性。为实现这个目的，二阶段提交算法的成立基于以下假设：</p>`,130),u=e("li",null,"该分布式系统中，存在一个节点作为协调者(Coordinator)，其他节点作为参与者(Cohorts)。且节点之间可以进行网络通信。",-1),p={href:"https://cloud.tencent.com/solution/cloudlog?from=10680",target:"_blank",rel:"noopener noreferrer"},v=e("li",null,[i("所有节点不会永久性损坏，即使损坏后仍然可以恢复。## "),e("strong",null,"第一阶段（投票阶段）")],-1),m=e("li",null,"协调者节点向所有参与者节点询问是否可以执行提交操作(vote)，并开始等待各参与者节点的响应。",-1),h=e("li",null,"参与者节点执行询问发起为止的所有事务操作，并将 Undo信息和 Redo信息写入日志。（注意：若成功这里其实每个参与者已经执行了事务操作）",-1),b=e("li",null,"各参与者节点响应协调者节点发起的询问。如果参与者节点的事务操作实际执行成功，则它返回一个”同意”消息；如果参与者节点的事务操作实际执行失败，则它返回一个”中止”消息。",-1),g=l('<p>**第二阶段（提交执行阶段）**当协调者节点从所有参与者节点获得的相应消息都为”同意”：</p><ul><li>协调者节点向所有参与者节点发出”正式提交(commit)”的请求。</li><li>参与者节点正式完成操作，并释放在整个事务期间内占用的资源。</li><li>参与者节点向协调者节点发送”完成”消息。</li><li>协调者节点受到所有参与者节点反馈的”完成”消息后，完成事务。如果任一参与者节点在第一阶段返回的响应消息为”中止”：</li><li>协调者节点向所有参与者节点发出”回滚操作(rollback)”的请求。</li><li>参与者节点利用之前写入的 Undo信息执行回滚，并释放在整个事务期间内占用的资源。</li><li>参与者节点向协调者节点发送”回滚完成”消息。</li><li>协调者节点受到所有参与者节点反馈的”回滚完成”消息后，取消事务。</li></ul><h2 id="请讲一下对-tcc-协议的理解" tabindex="-1"><a class="header-anchor" href="#请讲一下对-tcc-协议的理解"><span>请讲一下对 TCC 协议的理解</span></a></h2><p>Try Confirm Cancel</p><ul><li>Try：尝试待执行的业务，这个过程并未执行业务，只是完成所有业务的一致性检查，并预留好执行所需的全部资源。</li><li>Confirm：执行业务，这个过程真正开始执行业务，由于 Try 阶段已经完成了一致性检查，因此本过程直接执行，而不做任何检查。并且在执行的过程中，会使用到 Try 阶段预留的业务资源。</li><li>Cancel：取消执行的业务，若业务执行失败，则进入 Cancel 阶段，它会释放所有占用的业务资源，并回滚 Confirm 阶段执行的操作。</li></ul><h1 id="clickhouse-面试题" tabindex="-1"><a class="header-anchor" href="#clickhouse-面试题"><span>ClickHouse 面试题</span></a></h1><h2 id="什么是-clickhouse" tabindex="-1"><a class="header-anchor" href="#什么是-clickhouse"><span>什么是 ClickHouse？</span></a></h2><p>ClickHouse 是近年来备受关注的开源列式数据库管理系统，主要用于数据分析 （OLAP）领域。通过向量化执行以及对 cpu 底层指令集（SIMD）的使用，它 可以对海量数据进行并行处理，从而加快数据的处理速度。ClickHouse 从 OLAP 场景需求出发，定制开发了一套全新的高效列式存储引擎，并且实现了数据有序 存储、主键索引、稀疏索引、数据 Sharding、数据 Partitioning、TTL、主备复 制等丰富功能。</p><h2 id="clickhouse-有哪些应用场景" tabindex="-1"><a class="header-anchor" href="#clickhouse-有哪些应用场景"><span>ClickHouse 有哪些应用场景？</span></a></h2><ol><li>绝大多数请求都是用于读访问的；</li><li>数据需要以大批次（大于 1000 行）进行更新，而不是单行更新；</li><li>数据只是添加到数据库，没有必要修改；</li><li>读取数据时，会从数据库中提取出大量的行，但只用到一小部分列；</li><li>表很“宽”，即表中包含大量的列；</li><li>查询频率相对较低（通常每台服务器每秒查询数百次或更少）；</li><li>对于简单查询，允许大约 50 毫秒的延迟；</li><li>列的值是比较小的数值和短字符串（例如，每个 URL只有 60 个字节）；</li><li>在处理单个查询时需要高吞吐量（每台服务器每秒高达数十亿行）；</li><li>不需要事务；</li><li>数据一致性要求较低；</li><li>每次查询中只会查询一个大表。除了一个大表，其余都是小表；</li><li>查询结果显著小于数据源。即数据有过滤或聚合。返回结果不超过单个服务 器内存。</li></ol><h2 id="clickhouse-列式存储的优点有哪些" tabindex="-1"><a class="header-anchor" href="#clickhouse-列式存储的优点有哪些"><span>ClickHouse 列式存储的优点有哪些？</span></a></h2><ul><li>当分析场景中往往需要读大量行但是少数几个列时，在行存模式下，数据按</li></ul><p>行连续存储，所有列的数据都存储在一个 block 中，不参与计算的列在 IO 时也要全部读出，读取操作被严重放大。而列存模式下，只需要读取参与计 算的列即可，极大的减低了 IO cost，加速了查询。</p><ul><li>同一列中的数据属于同一类型，压缩效果显著。列存往往有着高达十倍甚至</li></ul><p>更高的压缩比，节省了大量的存储空间，降低了存储成本。</p><ul><li>更高的压缩比意味着更小的 data size，从磁盘中读取相应数据耗时更短。 - 自由的压缩算法选择。不同列的数据具有不同的数据类型，适用的压缩算法</li></ul><p>也就不尽相同。可以针对不同列类型，选择最合适的压缩算法。</p><ul><li>高压缩比，意味着同等大小的内存能够存放更多数据，系统 cache 效果更好。</li></ul><h2 id="clickhouse-的缺点是是什么" tabindex="-1"><a class="header-anchor" href="#clickhouse-的缺点是是什么"><span>ClickHouse 的缺点是是什么？</span></a></h2><ul><li><p>不支持事务，不支持真正的删除/更新； - 不支持二级索引；</p></li><li><p>join 实现与众不同；</p></li><li><p>不支持窗口功能；</p></li><li><p>元数据管理需要人为干预。</p></li></ul><h2 id="clickhouse-的架构是怎样的" tabindex="-1"><a class="header-anchor" href="#clickhouse-的架构是怎样的"><span>ClickHouse 的架构是怎样的？</span></a></h2><p>![](/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.002.jpeg)</p><p>ClickHouse 采用典型的分组式的分布式架构，其中：</p><ul><li>Shard。集群内划分为多个分片或分组（Shard 0 … Shard N），通过 Shard</li></ul><p>的线性扩展能力，支持海量数据的分布式存储计算。</p><ul><li>Node。每个 Shard 内包含一定数量的节点（Node，即进程），同一 Shard 内的节点互为副本，保障数据可靠。ClickHouse 中副本数可按需建设，且逻</li></ul><p>辑上不同 Shard 内的副本数可不同。</p><ul><li>ZooKeeper Service。集群所有节点对等，节点间通过 ZooKeeper 服务进</li></ul><p>行分布式协调。</p><h2 id="clickhouse-的逻辑数据模型" tabindex="-1"><a class="header-anchor" href="#clickhouse-的逻辑数据模型"><span>ClickHouse 的逻辑数据模型？</span></a></h2><p>从用户使用角度看，ClickHouse 的逻辑数据模型与关系型数据库有一定的相似： 一个集群包含多个数据库，一个数据库包含多张表，表用于实际存储数据。</p><p>![](/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.003.png)</p><h2 id="clickhouse-的核心特性" tabindex="-1"><a class="header-anchor" href="#clickhouse-的核心特性"><span>ClickHouse 的核心特性？</span></a></h2><ul><li>列存储：列存储是指仅从存储系统中读取必要的列数据，无用列不读取，速</li></ul><p>度非常快。ClickHouse 采用列存储，这对于分析型请求非常高效。一个典型 且真实的情况是，如果我们需要分析的数据有 50 列，而每次分析仅读取其 中的 5 列，那么通过列存储，我们仅需读取必要的列数据，相比于普通行</p><p>存，可减少 10 倍左右的读取、解压、处理等开销，对性能会有质的影响。</p><ul><li>向量化执行：在支持列存的基础上，ClickHouse 实现了一套面向 向量化处</li></ul><p>理 的计算引擎，大量的处理操作都是向量化执行的。相比于传统火山模型中 的逐行处理模式，向量化执行引擎采用批量处理模式，可以大幅减少函数调 用开销，降低指令、数据的 Cache Miss，提升 CPU 利用效率。并且 ClickHouse 可 利 用 SIMD 指 令 进 一 步 加 速 执 行 效 率 。 这 部 分 是 ClickHouse 优于大量同类 OLAP 产品的重要因素。</p><ul><li>编码压缩：由于 ClickHouse 采用列存储，相同列的数据连续存储，且底层</li></ul><p>数据在存储时是经过排序的，这样数据的局部规律性非常强，有利于获得更 高的数据压缩比。此外，ClickHouse 除了支持 LZ### ZSTD 等通用压缩算 法外，还支持 Delta、DoubleDelta、Gorilla 等专用编码算法，用于进一步 提高数据压缩比。</p><ul><li>多索引：列存用于裁剪不必要的字段读取，而索引则用于裁剪不必要的记录</li></ul><p>读取。ClickHouse 支持丰富的索引，从而在查询时尽可能的裁剪不必要的 记录读取，提高查询性能。</p><h2 id="使用-clickhouse-时有哪些注意点" tabindex="-1"><a class="header-anchor" href="#使用-clickhouse-时有哪些注意点"><span>使用 ClickHouse 时有哪些注意点？</span></a></h2><p><strong>分区和索引</strong> 分区粒度根据业务特点决定，不宜过粗或过细。一般选择按天分区，也可指定为</p><p>tuple()；以单表 1 亿数据为例，分区大小控制在 10-30 个为最佳。</p><p>必须指定索引列，clickhouse 中的索引列即排序列，通过 order by 指定，一般 在查询条件中经常被用来充当筛选条件的属性被纳入进来；可以是单一维度，也 可以是组合维度的索引；通常需要满足高级列在前、查询频率大的在前原则；还 有基数特别大的不适合做索引列，如用户表的 userid 字段；通常筛选后的数据 满足在百万以内为最佳。</p><p><strong>数据采样策略</strong> 通过采用运算可极大提升数据分析的性能。</p><p>数据量太大时应避免使用 select * 操作，查询的性能会与查询的字段大小和数 量成线性变换；字段越少，消耗的 IO 资源就越少，性能就会越高。 千万以上数据集用 order by 查询时需要搭配 where 条件和 limit 语句一起使用。</p><p>如非必须不要在结果集上构建虚拟列，虚拟列非常消耗资源浪费性能，可以考虑 在前端进行处理，或者在表中构造实际字段进行额外存储。 不建议在高基列上执行 distinct 去重查询，改为近似去重 uniqCombined。 多表 Join 时要满足小表在右的原则，右表关联时被加载到内存中与左表进行比 较。</p><p><strong>存储</strong></p><p>ClickHouse 不支持设置多数据目录，为了提升数据 一个券组绑定多块物理磁盘提升读写性能；多数查询场景 硬盘快 2-3 倍。</p><p>io 性能，可以挂载虚拟券组，</p><p>SSD 盘会比普通机械</p><h2 id="clickhouse-的引擎有哪些" tabindex="-1"><a class="header-anchor" href="#clickhouse-的引擎有哪些"><span>ClickHouse 的引擎有哪些？</span></a></h2><p>ClickHouse 提供了大量的数据引擎，分为数据库引擎、表引擎，根据数据特点 及使用场景选择合适的引擎至关重要。</p><p><strong>ClickHouse 引擎分类</strong></p><p>![](/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.004.png)</p><p>在以下几种情况下，ClickHouse 使用自己的数据库引擎：</p><ul><li><p>决定表存储在哪里以及以何种方式存储； - 支持哪些查询以及如何支持；</p></li><li><p>并发数据访问；</p></li><li><p>索引的使用；</p></li><li><p>是否可以执行多线程请求；</p></li><li><p>数据复制参数。</p></li></ul><p>在所有的表引擎中，最为核心的当属 MergeTree 系列表引擎，这些表引擎拥有 最为强大的性能和最广泛的使用场合。对于非 MergeTree 系列的其他引擎而言， 主要用于特殊用途，场景相对有限。而 MergeTree 系列表引擎是官方主推的存 储引擎，支持几乎所有 ClickHouse 核心功能。</p><p>MergeTree 作为家族系列最基础的表引擎，主要有以下特点：</p><ul><li><p>存储的数据按照主键排序：允许创建稀疏索引，从而加快数据查询速度； - 支持分区，可以通过 PRIMARY KEY语句指定分区字段；</p></li><li><p>支持数据副本；</p></li><li><p>支持数据采样。</p></li></ul><h2 id="建表引擎参数有哪些" tabindex="-1"><a class="header-anchor" href="#建表引擎参数有哪些"><span>建表引擎参数有哪些？</span></a></h2><p>**ENGINE：**ENGINE = MergeTree()，MergeTree 引擎没有参数。</p><p>**ORDER BY：**order by 设定了分区内的数据按照哪些字段顺序进行有序保 存。</p><p>order by 是 MergeTree 中唯一一个必填项，甚至比 primary key 还重要，因 为当用户不设置主键的情况，很多处理会依照 order by 的字段进行处理。 要求：主键必须是 order by 字段的前缀字段。</p><p>如果 ORDER BY与 PRIMARY KEY不同，PRIMARY KEY必须是 ORDER BY的 前缀(为了保证分区内数据和主键的有序性)。</p><p>ORDER BY 决定了每个分区中数据的排序规则;</p><p>PRIMARY KEY 决定了一级索引(primary.idx);</p><p>ORDER BY 可以指代 PRIMARY KEY, 通常只用声明 ORDER BY 即可。</p><p>**PARTITION BY：**分区字段，可选。如果不填：只会使用一个分区。 分区目录：MergeTree 是以列文件+索引文件+表定义文件组成的，但是如果设 定了分区那么这些文件就会保存到不同的分区目录中。</p><p>**PRIMARY KEY：**指定主键，如果排序字段与主键不一致，可以单独指定主 键字段。否则默认主键是排序字段。可选。</p><p>**SAMPLE BY：**采样字段，如果指定了该字段，那么主键中也必须包含该字 段。比如 SAMPLE BY intHash32(UserID) ORDER BY (CounterID, EventDate, intHash32(UserID))。可选。</p><p>**TTL：**数据的存活时间。在 MergeTree 中，可以为某个列字段或整张表设置 TTL。当时间到达时，如果是列字段级别的 TTL，则会删除这一列的数据；如果 是表级别的 TTL，则会删除整张表的数据。可选。</p><p>**SETTINGS：**额外的参数配置。可选。</p>',75);function k(f,y){const n=r("ExternalLinkIcon");return a(),d("div",null,[o,e("ul",null,[u,e("li",null,[i("所有节点都采用预写式日志，且日志被写入后即被保持在可靠的存储设备上，即使节点损坏不会导致日志数据"),e("a",p,[i("的消失。"),t(n)])]),v,m,h,b]),g])}const x=s(c,[["render",k],["__file","面试题.html.vue"]]),A=JSON.parse('{"path":"/%E6%9E%B6%E6%9E%84/%E5%88%86%E5%B8%83%E5%BC%8F/%E9%9D%A2%E8%AF%95%E9%A2%98.html","title":"分布式面试题","lang":"zh-CN","frontmatter":{"description":"分布式面试题 分布式服务接口的幂等性如何设计？ 所谓幂等性，就是说一个接口，多次发起同一个请求，你这个接口得保证结果是准确得。比如不能多扣款。不能多插入一条数据，不能将统计值多加了1，这就是幂等性。 其实保证幂等性主要是三点： 对于每个请求必须有一个唯一的标识，举个例子：订单支付请求，肯定得包含订单 ID，一个订单 ID最多支付一次。 每次处理完请求之...","head":[["meta",{"property":"og:url","content":"https://golangguide.top/%E6%9E%B6%E6%9E%84/%E5%88%86%E5%B8%83%E5%BC%8F/%E9%9D%A2%E8%AF%95%E9%A2%98.html"}],["meta",{"property":"og:site_name","content":"golang全栈指南"}],["meta",{"property":"og:title","content":"分布式面试题"}],["meta",{"property":"og:description","content":"分布式面试题 分布式服务接口的幂等性如何设计？ 所谓幂等性，就是说一个接口，多次发起同一个请求，你这个接口得保证结果是准确得。比如不能多扣款。不能多插入一条数据，不能将统计值多加了1，这就是幂等性。 其实保证幂等性主要是三点： 对于每个请求必须有一个唯一的标识，举个例子：订单支付请求，肯定得包含订单 ID，一个订单 ID最多支付一次。 每次处理完请求之..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://golangguide.top/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.002.jpeg"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2024-02-13T08:18:41.000Z"}],["meta",{"name":"twitter:card","content":"summary_large_image"}],["meta",{"name":"twitter:image:alt","content":"分布式面试题"}],["meta",{"property":"article:author","content":"小白debug"}],["meta",{"property":"article:modified_time","content":"2024-02-13T08:18:41.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"分布式面试题\\",\\"image\\":[\\"https://golangguide.top/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.002.jpeg\\",\\"https://golangguide.top/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.003.png\\",\\"https://golangguide.top/Users/zhaolun.lin/Documents/self/计算机基础/其他/Golang部分/Converted documents/clickhouse/Aspose.Words.98e920e6-df49-4cf7-8f32-24a01cb9421b.004.png\\"],\\"dateModified\\":\\"2024-02-13T08:18:41.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"小白debug\\",\\"url\\":\\"https://xiaobaidebug.top/\\"}]}"],["link",{"rel":"canonical","href":"https://golangguide.top/%E6%9E%B6%E6%9E%84/%E5%88%86%E5%B8%83%E5%BC%8F/%E9%9D%A2%E8%AF%95%E9%A2%98.html"}],["meta",{"property":"og:url","content":"https://golangguide.top/%E6%9E%B6%E6%9E%84/%E5%88%86%E5%B8%83%E5%BC%8F/%E9%9D%A2%E8%AF%95%E9%A2%98.html"}],["meta",{"property":"og:site_name","content":"golang全栈指南"}],["meta",{"property":"og:title","content":"分布式面试题"}],["meta",{"property":"og:description","content":"分布式面试题 分布式服务接口的幂等性如何设计？ 所谓幂等性，就是说一个接口，多次发起同一个请求，你这个接口得保证结果是准确得。比如不能多扣款。不能多插入一条数据，不能将统计值多加了1，这就是幂等性。 其实保证幂等性主要是三点： 对于每个请求必须有一个唯一的标识，举个例子：订单支付请求，肯定得包含订单 ID，一个订单 ID最多支付一次。 每次处理完请求之..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2024-02-13T08:18:41.000Z"}],["meta",{"property":"article:modified_time","content":"2024-02-13T08:18:41.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"分布式面试题\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2024-02-13T08:18:41.000Z\\",\\"author\\":[]}"]]},"headers":[{"level":2,"title":"分布式服务接口的幂等性如何设计？","slug":"分布式服务接口的幂等性如何设计","link":"#分布式服务接口的幂等性如何设计","children":[]},{"level":2,"title":"分布式系统中的接口调用如何保证顺序性？","slug":"分布式系统中的接口调用如何保证顺序性","link":"#分布式系统中的接口调用如何保证顺序性","children":[]},{"level":2,"title":"分布式锁实现原理，用过吗？","slug":"分布式锁实现原理-用过吗","link":"#分布式锁实现原理-用过吗","children":[]},{"level":2,"title":"Etcd怎么实现分布式锁?","slug":"etcd怎么实现分布式锁","link":"#etcd怎么实现分布式锁","children":[]},{"level":2,"title":"说说 ZooKeeper 一般都有哪些使用场景？","slug":"说说-zookeeper-一般都有哪些使用场景","link":"#说说-zookeeper-一般都有哪些使用场景","children":[]},{"level":2,"title":"说说你们的分布式 session 方案是啥？怎么做的？","slug":"说说你们的分布式-session-方案是啥-怎么做的","link":"#说说你们的分布式-session-方案是啥-怎么做的","children":[]},{"level":2,"title":"分布式事务了解吗？","slug":"分布式事务了解吗","link":"#分布式事务了解吗","children":[]},{"level":2,"title":"那常见的分布式锁有哪些解决方案？","slug":"那常见的分布式锁有哪些解决方案","link":"#那常见的分布式锁有哪些解决方案","children":[]},{"level":2,"title":"ZK 和 Redis 的区别，各自有什么优缺点？","slug":"zk-和-redis-的区别-各自有什么优缺点","link":"#zk-和-redis-的区别-各自有什么优缺点","children":[]},{"level":2,"title":"MySQL 如何做分布式锁？","slug":"mysql-如何做分布式锁","link":"#mysql-如何做分布式锁","children":[]},{"level":2,"title":"你了解业界哪些大公司的分布式锁框架","slug":"你了解业界哪些大公司的分布式锁框架","link":"#你了解业界哪些大公司的分布式锁框架","children":[]},{"level":2,"title":"请讲一下你对 CAP 理论的理解","slug":"请讲一下你对-cap-理论的理解","link":"#请讲一下你对-cap-理论的理解","children":[]},{"level":2,"title":"请讲一下你对 BASE 理论的理解","slug":"请讲一下你对-base-理论的理解","link":"#请讲一下你对-base-理论的理解","children":[]},{"level":2,"title":"分布式与集群的区别是什么？","slug":"分布式与集群的区别是什么","link":"#分布式与集群的区别是什么","children":[]},{"level":2,"title":"请讲一下 BASE 理论的三要素","slug":"请讲一下-base-理论的三要素","link":"#请讲一下-base-理论的三要素","children":[]},{"level":2,"title":"请说一下对两阶段提交协议的理解","slug":"请说一下对两阶段提交协议的理解","link":"#请说一下对两阶段提交协议的理解","children":[]},{"level":2,"title":"请讲一下对 TCC 协议的理解","slug":"请讲一下对-tcc-协议的理解","link":"#请讲一下对-tcc-协议的理解","children":[]},{"level":2,"title":"什么是 ClickHouse？","slug":"什么是-clickhouse","link":"#什么是-clickhouse","children":[]},{"level":2,"title":"ClickHouse 有哪些应用场景？","slug":"clickhouse-有哪些应用场景","link":"#clickhouse-有哪些应用场景","children":[]},{"level":2,"title":"ClickHouse 列式存储的优点有哪些？","slug":"clickhouse-列式存储的优点有哪些","link":"#clickhouse-列式存储的优点有哪些","children":[]},{"level":2,"title":"ClickHouse 的缺点是是什么？","slug":"clickhouse-的缺点是是什么","link":"#clickhouse-的缺点是是什么","children":[]},{"level":2,"title":"ClickHouse 的架构是怎样的？","slug":"clickhouse-的架构是怎样的","link":"#clickhouse-的架构是怎样的","children":[]},{"level":2,"title":"ClickHouse 的逻辑数据模型？","slug":"clickhouse-的逻辑数据模型","link":"#clickhouse-的逻辑数据模型","children":[]},{"level":2,"title":"ClickHouse 的核心特性？","slug":"clickhouse-的核心特性","link":"#clickhouse-的核心特性","children":[]},{"level":2,"title":"使用 ClickHouse 时有哪些注意点？","slug":"使用-clickhouse-时有哪些注意点","link":"#使用-clickhouse-时有哪些注意点","children":[]},{"level":2,"title":"ClickHouse  的引擎有哪些？","slug":"clickhouse-的引擎有哪些","link":"#clickhouse-的引擎有哪些","children":[]},{"level":2,"title":"建表引擎参数有哪些？","slug":"建表引擎参数有哪些","link":"#建表引擎参数有哪些","children":[]}],"git":{"createdTime":1707812321000,"updatedTime":1707812321000,"contributors":[{"name":"xiaobai","email":"xiaobaidebug@gmail.com","commits":1}]},"readingTime":{"minutes":34.73,"words":10420},"filePathRelative":"架构/分布式/面试题.md","localizedDate":"2024年2月13日","autoDesc":true}');export{x as comp,A as data};
